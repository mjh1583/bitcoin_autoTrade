...
access = "bpuqrdNfkU6Z9jCDdfS7RxBj5xZsLtxVmpAuv4L6"
secret = "QjEuvoNnZ9MABI1HHYvFEpcxSeKOjb6JIUnih5Z6"
    ...
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            if target_price < current_price and ma15 < current_price:
                krw = get_balance("KRW")
                if krw > 5000:
                    buy_result = upbit.buy_market_order("KRW-ETH", krw)
                    post_message(myToken,"#bitcoinautotrade", "ETH buy : " +str(buy_result))
        else:
            btc = get_balance("ETH")
            if btc > 0.00015:
                sell_result = upbit.sell_market_order("KRW-ETH", btc)
                post_message(myToken,"#bitcoinautotrade", "ETH sell : " +str(sell_result))
        time.sleep(1)
